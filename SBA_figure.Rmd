...
sba$Form<-ifelse(sba$Variation!="Old Form","New Form",sba$Variation)
```
...
```{r figConversion}
p_sba_conv<-ggplot(newdat,aes(x=Date,y=ConversionRate,col=Form))+geom_line()+
...
```{r figSomeInfo}
p_sba_conv<-ggplot(newdat,aes(x=Date,y=SomeInfo,col=Form))+geom_line()+
